...
UnitTest::Suite TestNumericsVectorsSuite {
	...
	{"Geometric Comparisons",
		[](UnitTest::Examiner * examiner) {
			...
			examiner->check(vec(4, 5).less_than(vec(6, 6))) << "Vector comparison correct";
			examiner->check(!vec(4, 5).less_than(vec(4, 4))) << "Vector comparison correct";

			examiner->check(vec(4, 5).less_than_or_equal(vec(6, 6))) << "Vector comparison correct";
			examiner->check(vec(4, 4).less_than_or_equal(vec(4, 4))) << "Vector comparison correct";
			examiner->check(!vec(4, 5).less_than_or_equal(vec(4, 4))) << "Vector comparison correct";

			examiner->check(!vec(4, 5).greater_than(vec(6, 6))) << "Vector comparison correct";
			examiner->check(!vec(4, 5).greater_than(vec(4, 4))) << "Vector comparison correct";
			examiner->check(vec(5, 5).greater_than(vec(4, 4))) << "Vector comparison correct";

			examiner->check(!vec(4, 5).greater_than_or_equal(vec(6, 6))) << "Vector comparison correct";
			examiner->check(vec(4, 5).greater_than_or_equal(vec(4, 4))) << "Vector comparison correct";
			examiner->check(vec(4, 4).greater_than_or_equal(vec(4, 4))) << "Vector comparison correct";
		}
	},
	...
	{"Comparisons"",
		[](UnitTest::Examiner * examiner) {
			...
			examiner->check(vec(2, 1) % 2 == vec(0, 1)) << "% ==";

			testing("Advanced Arithemtic");
			examiner->check(v2.sum() == 3) << "sum";
			examiner->check(v4.product() == 4) << "product";
			examiner->check(vec(2.5, 5.5).floor() == vec(2.0, 5.0)) << "floor";
			examiner->check(vec(2.5, 5.5).ceil() == vec(3.0, 6.0)) << "ceil";
			examiner->check(vec(2.5, 5.25).frac() == vec(0.5, 0.25)) << "frac";
			...
			examiner->check(vec(0.0, 0.0, 2.5).normalize() == vec(0.0, 0.0, 1.0)) << "Normalized correctly";
			examiner->check(vec(0.0, 0.0, 2.5).normalize(2) == vec(0.0, 0.0, 2.0)) << "Normalized correctly";
